...
import { analyzeDocument, validateExtractedData, type DocumentAnalysis } from "./ai-extraction";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/documents/analyze', upload.single('file'), async (req: any, res) => {
    try {
      const file = req.file;
      if (!file) {
      ...
      // Demo mode - return mock analysis for testing
      if (!process.env.GOOGLE_CLIENT_ID) {
        const mockAnalysis = {
          documentType: file.originalname.includes('insurance') ? 'Insurance Policy' : 
                      file.originalname.includes('bank') ? 'Bank Statement' :
                      file.originalname.includes('tax') ? 'Tax Document' :
                      file.originalname.includes('license') ? 'Driver License' : 'Financial Document',
          category: file.originalname.includes('insurance') ? 'Insurance' : 
                   file.originalname.includes('bank') ? 'Finance' :
                   file.originalname.includes('tax') ? 'Taxes' :
                   file.originalname.includes('license') ? 'Family IDs' : 'Finance',
          extractedData: {
            category: file.originalname.includes('insurance') ? 'Insurance' : 'Finance',
            subcategory: file.originalname.includes('insurance') ? 'Auto Insurance' : 'Bank Accounts',
            fields: file.originalname.includes('insurance') ? {
            ...
            confidence: 0.92,
            summary: file.originalname.includes('insurance') ? 
              'Auto insurance policy with State Farm providing comprehensive coverage with $100,000 limit and $500 deductible.' :
              'Monthly bank statement showing checking account balance and recent transactions.',
            suggestedFileName: file.originalname.includes('insurance') ? 
              'Auto_Insurance_Policy_2024.pdf' : 'Chase_Statement_June_2024.pdf'
          },
        ...
        return res.json({
          ...
          message: 'Document analyzed successfully (Demo Mode)'
        });
      }

      // Production mode with actual AI analysis
      const user = req.user as AuthenticatedUser;
      if (!user) {
        return res.status(401).json({ error: 'Authentication required' });
      }
  ...
  // Document upload with confirmed extracted data
  app.post('/api/documents/upload', requireAuth, upload.single('file'), async (req: any, res) => {
    try {
      const user = req.user as AuthenticatedUser;
      const { category, categoryFolderId, extractedData, userCorrections } = req.body;
      const file = req.file;
      ...
      // If we have extracted data, validate and save it to Google Sheets
      if (extractedData) {
        const validatedData = await validateExtractedData(
          JSON.parse(extractedData),
          userCorrections ? JSON.parse(userCorrections) : {}
        );

        // Save structured data to appropriate Google Sheets category
        const sheetsService = new GoogleSheetsService(user);
        const spreadsheetId = req.body.spreadsheetId; // This should come from the user's vault setup
        
        if (spreadsheetId) {
          await sheetsService.addData(spreadsheetId, validatedData.category, [
            new Date().toISOString(),
            validatedData.suggestedFileName || file.originalname,
  ...
  app.get('/api/documents/:documentId/relationships', async (req: any, res) => {
    try {
      const { documentId } = req.params;
      const { category } = req.query;

      // Demo mode - return mock relationships
      if (!process.env.GOOGLE_CLIENT_ID) {
        const { DocumentRelationshipService } = await import('./document-relationships');
        const mockUser = { access_token: 'demo' };
        const relationshipService = new DocumentRelationshipService(mockUser);
        
        const relationships = await relationshipService.getDocumentRelationships(documentId, category);
      ...
      // Production mode with actual Google API integration
      const user = req.user as AuthenticatedUser;
      if (!user) {
        return res.status(401).json({ error: 'Authentication required' });
      }
      ...
      const { DocumentRelationshipService } = await import('./document-relationships');
      const relationshipService = new DocumentRelationshipService(user);
  ...
  app.get('/api/documents/:documentId/impact', async (req: any, res) => {
    try {
      const { documentId } = req.params;
      const { category } = req.query;

      // Demo mode - return mock impact analysis
      if (!process.env.GOOGLE_CLIENT_ID) {
        const { DocumentRelationshipService } = await import('./document-relationships');
        const mockUser = { access_token: 'demo' };
        const relationshipService = new DocumentRelationshipService(mockUser);
        
        const impact = await relationshipService.analyzeDocumentImpact(documentId, category, {});
      ...
      console.error('Error analyzing document impact:', error);
